...
bool FFFMPEGMediaPlayer::Open(const TSharedRef<FArchive, ESPMode::ThreadSafe>& Archive, const FString& OriginalUrl,
                              const IMediaOptions* /*Options*/)
{
	...
	{
		UE_LOG(LogFFMPEGMedia, Verbose, TEXT("Player %p: Cannot open media from archive (no original URL provided)"),
		       this);
		return false;
	}
...
bool FFFMPEGMediaPlayer::InitializePlayer(const TSharedPtr<FArchive, ESPMode::ThreadSafe>& Archive, const FString& Url,
                                          bool Precache, const FMediaPlayerOptions* PlayerOptions)
{
	UE_LOG(LogFFMPEGMedia, Verbose, TEXT("Player %llx: Initializing %s (archive = %s, precache = %s)"), this, *Url,
	       Archive.IsValid() ? TEXT("yes") : TEXT("no"), Precache ? TEXT("yes") : TEXT("no"));
	...
	TFunction<void()> Task = [Archive, Url, Precache, PlayerOptions, TracksPtr = TWeakPtr<
			FFFMPEGMediaTracks, ESPMode::ThreadSafe>(Tracks), ThisPtr=this]()
	{
...
AVFormatContext* FFFMPEGMediaPlayer::ReadContext(const TSharedPtr<FArchive, ESPMode::ThreadSafe>& Archive,
...
static void print_option(const AVClass* clazz, const AVOption* o)
{
	...
	switch (o->type)
	{
	case AV_OPT_TYPE_BINARY: type = TEXT("hexadecimal string");
		break;
	case AV_OPT_TYPE_STRING: type = TEXT("string");
		break;
	case AV_OPT_TYPE_INT:
	case AV_OPT_TYPE_INT64: type = TEXT("integer");
		break;
	case AV_OPT_TYPE_FLOAT:
	case AV_OPT_TYPE_DOUBLE: type = TEXT("float");
		break;
	case AV_OPT_TYPE_RATIONAL: type = TEXT("rational number");
		break;
	case AV_OPT_TYPE_FLAGS: type = TEXT("flags");
		break;
	case AV_OPT_TYPE_BOOL: type = TEXT("bool");
		break;
	case AV_OPT_TYPE_SAMPLE_FMT: type = TEXT("SampleFmt");
		break;
	...
	if (o->unit)
	{
		UE_LOG(LogFFMPEGMedia, Display, TEXT("\t%s - %s: %s %s"), *type, UTF8_TO_TCHAR(o->name), *help,
		       *possibleValues);
	}
	else
	{
...
void FFFMPEGMediaPlayer::dumpFFMPEGInfo()
{
	...
	if (FormatContext)
	{
		...
		if (FormatContext->start_time != AV_NOPTS_VALUE)
		{
			...
			sz_duration += FString::Printf(TEXT("%s%d.%06d"), FormatContext->start_time >= 0 ? TEXT("") : TEXT("-"),
