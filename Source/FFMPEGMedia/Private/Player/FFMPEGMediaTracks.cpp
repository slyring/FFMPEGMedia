...
void FFFMPEGMediaTracks::Initialize(AVFormatContext* ic, const FString& Url, const FMediaPlayerOptions* PlayerOptions)
{
	...
	if (!AllStreamsAdded)
	{
		UE_LOG(LogFFMPEGMedia, Verbose, TEXT("Tracks %p: Not all available streams were added to the track collection"),
		       this);
	}
...
bool FFFMPEGMediaTracks::FetchAudio(TRange<FTimespan> TimeRange,
...
bool FFFMPEGMediaTracks::FetchCaption(TRange<FTimespan> TimeRange,
...
bool FFFMPEGMediaTracks::FetchMetadata(TRange<FTimespan> TimeRange,
...
bool FFFMPEGMediaTracks::FetchVideo(TRange<FTimespan> TimeRange,
...
bool FFFMPEGMediaTracks::GetAudioTrackFormat(int32 TrackIndex, int32 FormatIndex,
...
bool FFFMPEGMediaTracks::GetVideoTrackFormat(int32 TrackIndex, int32 FormatIndex,
...
bool FFFMPEGMediaTracks::SelectTrack(EMediaTrackType TrackType, int32 TrackIndex)
{
	...
	UE_LOG(LogFFMPEGMedia, Verbose, TEXT("Tracks %p: Selecting %s track %i"), this,
	       *MediaUtils::TrackTypeToString(TrackType), TrackIndex);
...
{
	UE_LOG(LogFFMPEGMedia, Verbose, TEXT("Tracks %p: Setting format on %s track %i to %i"), this,
	       *MediaUtils::TrackTypeToString(TrackType), TrackIndex, FormatIndex);
...
{
	UE_LOG(LogFFMPEGMedia, Verbose, TEXT("Tracks %p: Setting frame rate on format %i of video track %i to %f"), this,
	       FormatIndex, TrackIndex, FrameRate);
...
bool FFFMPEGMediaTracks::AddStreamToTracks(uint32 StreamIndex, bool IsVideoDevice,
                                           const FMediaPlayerTrackOptions& TrackOptions, FString& OutInfo)
{
	...
	if (MediaType == AVMEDIA_TYPE_AUDIO)
	{
		int samples = FFMAX(AUDIO_MIN_BUFFER_SIZE,
		                    2 << av_log2(CodecParams->sample_rate / AUDIO_MAX_CALLBACKS_PER_SEC));
		...
		Track->Format = {
			...
			TypeName,
			{
				...
				AV_SAMPLE_FMT_S16,
				(uint32)av_samples_get_buffer_size(NULL, CodecParams->channels, CodecParams->sample_rate,
				                                   AV_SAMPLE_FMT_S16, 1),
	...
	Track->DisplayName = (Track->Name.IsEmpty())
		                     ? FText::Format(
			                     LOCTEXT("UnnamedStreamFormat", "Unnamed Track (Stream {0})"),
			                     FText::AsNumber((uint32)StreamIndex))
...
int FFFMPEGMediaTracks::StreamHasEnoughPackets(AVStream* st, int stream_id, FFMPEGPacketQueue* queue)
{
	return stream_id < 0 ||
		...
		queue->GetNumPackets() > MIN_FRAMES && (!queue->GetDuration() || av_q2d(st->time_base) * queue->GetDuration() >
			1.0);
}
...
int FFFMPEGMediaTracks::StreamComponentOpen(int stream_index)
{
			...
			if (avcodec_open2(avctx2, hwCodec, NULL) >= 0)
			{
				...
				while (lastSelection >= 0)
				{
					...
					else
					{
						...
						const char* type_name = av_hwdevice_get_type_name(type);
						UE_LOG(LogFFMPEGMedia, Display, TEXT("Using hardware context type: %s"),
						       UTF8_TO_TCHAR(type_name));
	...
	{
		UE_LOG(LogFFMPEGMedia, Warning, TEXT("The maximum value for lowres supported by the decoder is %d"),
		       codec->max_lowres);
	...
	if (avctx->codec_type == AVMEDIA_TYPE_VIDEO)
	{
		thread_count = Settings->VideoThreads;
	}
	if (avctx->codec_type == AVMEDIA_TYPE_AUDIO)
	{
		thread_count = Settings->AudioThreads;
	}
	...
	case AVMEDIA_TYPE_AUDIO:
		...
		if ((FormatContext->iformat->flags & (AVFMT_NOBINSEARCH | AVFMT_NOGENSEARCH | AVFMT_NO_BYTE_SEEK)) && !
			FormatContext->iformat->read_seek)
		{
...
int FFFMPEGMediaTracks::UploadTexture(FFMPEGFrame* vp, AVFrame* frame, struct SwsContext** img_convert_ctx)
{
	...
	*img_convert_ctx = sws_getCachedContext(*img_convert_ctx,
	                                        ...
	                                        frame->height, AV_PIX_FMT_BGRA, SWS_BICUBIC, NULL, NULL, NULL);
					...
					if (!sp->IsUploaded())
					{
						...
						for (i = 0; i < (int)sp->GetSub().num_rects; i++)
						{
							...
							if (sub_rect->type == SUBTITLE_TEXT || sub_rect->type == SUBTITLE_ASS)
							{
								...
								if (CaptionSample->Initialize(
									sub_rect->type == SUBTITLE_TEXT ? sub_rect->text : sub_rect->ass,
									FVector2D(sub_rect->x, sub_rect->y), Time, CurrentDuration))
								{
...
int FFFMPEGMediaTracks::AudioDecodeFrame(FTimespan& time, FTimespan& duration)
{
	...
	do
	{
		...
		{
			if ((av_gettime_relative() - audioCallbackTime) > 1000000LL * targetAudio.HardwareSize / targetAudio.
				BytesPerSec / 2)
			{
				return -1;
			}
	...
	data_size = av_samples_get_buffer_size(NULL, af->GetFrame()->channels, af->GetFrame()->nb_samples,
	                                       (AVSampleFormat)af->GetFrame()->format, 1);

	dec_channel_layout =
	(af->GetFrame()->channel_layout && af->GetFrame()->channels ==
		av_get_channel_layout_nb_channels(af->GetFrame()->channel_layout))
		? af->GetFrame()->channel_layout
		: av_get_default_channel_layout(af->GetFrame()->channels);
		...
		(wanted_nb_samples != af->GetFrame()->nb_samples && !swrContext))
	{
		...
		swrContext = swr_alloc_set_opts(NULL,
		                                ...
		                                dec_channel_layout, (AVSampleFormat)af->GetFrame()->format,
		                                af->GetFrame()->sample_rate,
		                                0, NULL);
		if (!swrContext || swr_init(swrContext) < 0)
		{
			UE_LOG(LogFFMPEGMedia, Error,
			       ...
			       af->GetFrame()->sample_rate,
			       UTF8_TO_TCHAR( av_get_sample_fmt_name((AVSampleFormat)af->GetFrame()->format)),
			       af->GetFrame()->channels,
			       targetAudio.SampleRate, UTF8_TO_TCHAR(av_get_sample_fmt_name(targetAudio.Format)),
			       targetAudio.NumChannels);
			swr_free(&swrContext);
			return -1;
		}
	...
	if (swrContext)
	{
		...
		if (wanted_nb_samples != af->GetFrame()->nb_samples)
		{
			if (swr_set_compensation(
				swrContext,
				(wanted_nb_samples - af->GetFrame()->nb_samples) * targetAudio.SampleRate / af->GetFrame()->sample_rate,
...
void FFFMPEGMediaTracks::RenderAudio()
{
	...
	else
	{
		...
		{
			FScopeLock Lock(&CriticalSection);
			const TSharedRef<FFFMPEGMediaAudioSample, ESPMode::ThreadSafe> AudioSample = AudioSamplePool->
				AcquireShared();

			if (AudioSample->Initialize((uint8_t*)audioBuf, len1, targetAudio.NumChannels, targetAudio.SampleRate, time,
			                            duration))
			{
	...
	if (!isnan(audioClock))
	{
		audclk.SetAt(audioClock - (double)(2 * targetAudio.HardwareSize + audioBufSize) / targetAudio.BytesPerSec,
				...
				if (subTitleStream)
				{
					while (subpq.GetNumRemaining() > 0)
					{
						...
						if (sp->GetSerial() != subtitleq.GetSerial()
							...
							|| (sp2 && vidclk.GetPts() > (sp2->GetPts() + ((float)sp2->GetSub().start_display_time /
								1000))))
						{
							subpq.Next();
						}
