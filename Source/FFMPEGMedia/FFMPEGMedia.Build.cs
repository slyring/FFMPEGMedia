...
public class FFMPEGMedia : ModuleRules
{
	...
	private string UProjectPath => Directory.GetParent(ModulePath).Parent.FullName;


	private void CopyToBinaries(string Filepath, ReadOnlyTargetRules Target)
	{
		var binariesDir = Path.Combine(UProjectPath, "Binaries", Target.Platform.ToString());
	...
	public bool LoadFFmpeg(ReadOnlyTargetRules Target)
	{
		...
		if (Target.Platform == UnrealTargetPlatform.Win64)
		{
			...
			var PlatformString = Target.Platform == UnrealTargetPlatform.Win64 ? "x64" : "Win32";
			var LibrariesPath = Path.Combine(Path.Combine(Path.Combine(ThirdPartyPath, "ffmpeg", "lib"), "vs"),
				PlatformString);
			...
			string[] dlls =
			{
				...
				"swresample-3.dll", "swscale-5.dll", "postproc-55.dll"
			};

			var BinariesPath = Path.Combine(Path.Combine(Path.Combine(ThirdPartyPath, "ffmpeg", "bin"), "vs"),
				PlatformString);
		...
		else if (Target.Platform == UnrealTargetPlatform.Mac)
		{
			...
			var LibrariesPath = "/usr/local/lib";
			System.Console.WriteLine("... LibrariesPath -> " + LibrariesPath);

			string[] libs =
			{
				"libavcodec.58.dylib", "libavdevice.58.dylib", "libavfilter.7.dylib", "libavformat.58.dylib",
				"libavutil.56.dylib", "libswresample.3.dylib", "libswscale.5.dylib", "libpostproc.55.dylib"
			};
			foreach (var lib in libs) PublicAdditionalLibraries.Add(Path.Combine(LibrariesPath, lib));
		...
		else if (Target.Platform == UnrealTargetPlatform.Android)
		{
			...
			var LibrariesPath = Path.Combine(Path.Combine(ThirdPartyPath, "ffmpeg", "lib"), "android");
			...
			var finalPath = Path.Combine(ModulePath, "FFMPEGMedia_APL.xml");
